...
def Star_Alignment(N, Sequences, s_match, s_mismatch, s_gap):
        ...
        for j in range(N):
            if i != j:
                Similarity_Matrix[i][j] = global_align(
                    Sequences[i], Sequences[j], s_match, s_mismatch, s_gap)[2]
    ...
    for seq in Seqs_in_Orde_To_Align:

        Answ = global_align(
            seq, Star_Sequence, s_match, s_mismatch, s_gap)
        ...
        while i < len(Star_Sequence) and j < len(Temp):
            if Temp[j] == Star_Sequence[i]:
                i += 1
                j += 1
        ...
        last_gaps = len(Temp) - len(Star_Sequence) - len(new_gapes)

        for k in range(counter):
            for loc in new_gapes:
                Aligned_Sequences[Alignment_Order[k]] = Aligned_Sequences[Alignment_Order[k]
                                                                          ][0: loc] + '-' + Aligned_Sequences[Alignment_Order[k]][loc:]
...
def Block(N, Sequences):
    ...
    for i in range(len(Blocked_Seqs)):
        ...
        New_Block = Star_Alignment(
            N, Blocked_Seqs[i], s_match, s_mismatch, s_gap)

        if if_first != 0:
            for ll in range(len(Locations)):
                Locations[ll][0], Locations[ll][1] = Locations[ll][0] + \
                    Delta, Locations[ll][1] + Delta

        for j in range(N):
            Temp_Seq[j] = Temp_Seq[j][0:Locations[i][0] + 1] + \
                New_Block[j] + Temp_Seq[j][Locations[i][1]:]

        if MSA_Score(N, Sequences) < MSA_Score(N, Temp_Seq):
            Delta = len(Temp_Seq[0]) - len(Sequences[0])
            Sequences = Temp_Seq
            if_first += 1
...
Aligned_Sequences = Star_Alignment(
    N, Sequences, s_match, s_mismatch, s_gap)


while True:
    Temp = Block(N, Aligned_Sequences)

    if Temp == Aligned_Sequences:
        break

    Aligned_Sequences = Temp
